...
window.onload = function() {
    field = document.getElementById('field'); layer = document.getElementById('pieces-layer');
    canvas = document.getElementById('drawLayer'); ctx = canvas.getContext('2d');
    initCanvas(); resetBoard(); loadSavedPlays();
    window.addEventListener('resize', initCanvas);

    canvas.addEventListener('pointerdown', (e) => {
        if (!drawMode) return;
        isDrawing = true; ctx.beginPath(); ctx.strokeStyle = currentColor; ctx.lineWidth = 4; ctx.lineCap = 'round';
...
function initCanvas() { canvas.width = field.clientWidth; canvas.height = field.clientHeight; }

function handleFormationChange() {
    const val = document.getElementById('formationSelect').value;
    if (!val) return;
    if (val.startsWith("CUSTOM_")) applyCustomTactic(val.replace("CUSTOM_", ""));
    else applyTactics();
}

function applyTactics() {
    const formation = document.getElementById('formationSelect').value;
    let size = ["3-2-3", "3-4-1-flat"].includes(formation) ? "9v9" : "11v11";
    ...
    for (let n = 1; n <= 11; n++) {
        if (redMap[n]) {
            redTeam[n].style.display = 'flex'; blueTeam[n].style.display = 'flex';
            ...
            redTeam[n].style.display = 'none'; blueTeam[n].style.display = 'none';
        }
    }
}

function saveCustomTactic() {
    const name = prompt("Name your play:"); if (!name) return;
    const w = field.clientWidth, h = field.clientHeight;
    ...
    const ball = document.querySelector('.ball'); if (ball) snap.ball = getPos(ball);
    const saved = JSON.parse(localStorage.getItem('gafferPlays') || '{}');
    saved[name] = snap; localStorage.setItem('gafferPlays', JSON.stringify(saved));
    loadSavedPlays();
}
...
function deleteCustomTactic() {
    ...
    if (confirm(`Delete '${name}'?`)) {
        const saved = JSON.parse(localStorage.getItem('gafferPlays') || '{}');
        delete saved[name]; localStorage.setItem('gafferPlays', JSON.stringify(saved));
        loadSavedPlays(); select.value = "";
    }
}

function loadSavedPlays() {
    const group = document.getElementById('customGroup'); group.innerHTML = "";
    const saved = JSON.parse(localStorage.getItem('gafferPlays') || '{}');
    for (let name in saved) {
        const opt = document.createElement('option'); opt.value = "CUSTOM_" + name; opt.innerText = name;
...
function applyCustomTactic(name) {
    const saved = JSON.parse(localStorage.getItem('gafferPlays') || '{}');
    const data = saved[name]; const w = field.clientWidth, h = field.clientHeight;
    const move = (el, pos) => { el.style.left = (pos[0] * w) + 'px'; el.style.top = (pos[1] * h) + 'px'; };
    for (let i in data.red) move(redTeam[i], data.red[i]);
    for (let i in data.blue) move(blueTeam[i], data.blue[i]);
    const ball = document.querySelector('.ball'); if (ball) move(ball, data.ball);
}

function createPiece(num, color, x, y) {
    const p = document.createElement('div'); p.className = 'piece ' + color;
    if (num === 'ball') p.classList.add('ball'); else p.innerText = num;
    p.style.left = x + 'px'; p.style.top = y + 'px';
    p.onpointerdown = (e) => {
        e.preventDefault(); p.style.transition = 'none'; p.setPointerCapture(e.pointerId);
        p.onpointermove = (ev) => {
            const rect = field.getBoundingClientRect();
            p.style.left = (ev.clientX - rect.left - 19) + 'px'; p.style.top = (ev.clientY - rect.top - 19) + 'px';
        };
        p.onpointerup = () => {
            p.onpointermove = null;
            setTimeout(() => { p.style.transition = 'left 0.6s cubic-bezier(0.23, 1, 0.32, 1), top 0.6s cubic-bezier(0.23, 1, 0.32, 1)'; }, 10);
        };
    };
...
function resetBoard() {
    layer.innerHTML = ""; const w = field.clientWidth;
    for (let i = 1; i <= 11; i++) {
